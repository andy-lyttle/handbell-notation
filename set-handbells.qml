            ...
            if(allNotes["Unknown"].length) {
                  ...
                  if(fullScore) {
                        if(allNotes["Handbells"].length == 1) {
                              DialogBox.showWarning("We only found one note in the score, which already has a normal note head.  Nothing to do.");
                        } else {
                              DialogBox.showWarning("All " + allNotes["Handbells"].length + " notes in the score already have normal note heads.  Nothing to do.");
                        }
                  } else {
                        if(allNotes["Handbells"].length == 1) {
                              DialogBox.showWarning("The only selected note already has a normal note head.  Nothing to do.");
                        } else {
                              DialogBox.showWarning("All " + allNotes["Handbells"].length + " selected notes already have normal note heads.  Nothing to do.");
                        }
                  }
            } else {
